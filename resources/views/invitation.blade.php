...
@section('content')
    ...
    $linkWA = "https://wa.me/6285157702192?text=" . $pesanWA;
                            ...
                            <div class="rsvp-section text-center p-6 bg-white rounded-lg shadow-lg">
                                <h3 class="text-xl font-bold mb-4">Konfirmasi Kehadiran</h3>
                                <div class="flex flex-wrap justify-center gap-4">
                                    <button onclick="sendRSVP('Hadir')"
                                        class="px-6 py-2 bg-green-600 text-white rounded-full hover:bg-green-700 transition">Hadir</button>
                                    <button onclick="sendRSVP('Tidak Hadir')"
                                        class="px-6 py-2 bg-red-600 text-white rounded-full hover:bg-red-700 transition">Tidak Hadir</button>
                                </div>
                                <p id="rsvp-status" class="mt-4 text-gray-600 italic">Status saat ini: Memuat...</p>
                            </div>

                            <script>
                                // Di dalam file invitation.blade.php bagian script

                                    function sendRSVP(status) {
                                        // Pastikan status dikonversi ke HURUF BESAR agar sesuai dengan dropdown di Sheets
                                        const statusUpperCase = status.toUpperCase();

                                        const pathArray = window.location.pathname.split('/');
                                        const guestID = pathArray[pathArray.length - 1];

                                        document.getElementById('rsvp-status').innerText = 'Sedang mengirim...';
                                        ...
                                        fetch('/update-attendance', {
                                            ...
                                            body: JSON.stringify({
                                                id: guestID,
                                                status: statusUpperCase // Mengirim HADIR atau TIDAK HADIR
                                            })
                                        })
                                            .then(response => {
                                                if (!response.ok) {
                                                    // Jika error (misal 500 atau 419), lempar ke bagian catch
                                            ...
                                            .then(data => {
                                                if (data.success) {
                                                    document.getElementById('rsvp-status').innerText = 'Konfirmasi ' + data.status + ' berhasil tersimpan.';
                                                }
                                            })
                                            .catch(error => {
                                                console.error('Error Detail:', error);
                                                document.getElementById('rsvp-status').innerText = 'Gagal mengirim. Coba lagi.';
                                            });
                                    }
                            </script>
                        </section>
